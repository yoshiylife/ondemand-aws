...
# Set security groups
query=$"SecurityGroupRules[?GroupId==\`$sgid\` && CidrIpv4==\`$ipaddress/32\` && contains(Description,\`$host-ondemand\`)].SecurityGroupRuleId"
...
if [[ -z $"$output" ]]; then
    ...
    auth_egress=$"IpProtocol=-1,FromPort=-1,ToPort=-1,IpRanges=[{CidrIp=$ipaddress/32,Description=$host-ondemand}]"
